...
2021-12-20:
1. 修复了时间模块相关的bug, sleep/standby, rtc模块正常工作。
2. 硬件 SPI 完成。
3. WDT 工作了。
   
2021-12-21：
由于无法取得 WM806_SDK 授权，项目中止。
WM806 SDK 一直处于授权未声明状态，因此无法确定其是否与 MicroPython 的MIT开源协议相容, 我们不会在无法预期的结果上耗费精力，决定中止此项目。

此项目中代码在 MIT 协议下发布，使用的 WM806 SDK 授权不明。
此项目声明的 MIT 协议并不能覆盖 WM806 SDK 版权，也不能对其权属做任何暗示或明示的定义。
使用者如能取得 WM806 SDK 授权，请依据 WM806 SDK 授权，自行判定其是否与此项目使用的 MIT 协议相容。
